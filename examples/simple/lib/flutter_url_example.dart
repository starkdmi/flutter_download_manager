...
class _ExampleAppState extends State<ExampleApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text("DownloadUrlWidget"),
          actions: [
            ...
            builder: (context, controller, state, progress, error, request) {
              switch (state) {
                case DownloadWidgetState.initial:
                  return IconButton(onPressed: controller.download, icon: const Icon(Icons.download_rounded));
                ...
                case DownloadWidgetState.downloading:
                  return Stack(alignment: Alignment.center, children: [
                    ...
                    IconButton(onPressed: controller.cancel, icon: const Icon(Icons.close_rounded)),
                  ]);
                case DownloadWidgetState.failed:
                  return IconButton(onPressed: controller.download, icon: const Icon(Icons.restart_alt_rounded));
